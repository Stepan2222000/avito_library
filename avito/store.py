...
class Склад:
    ...
    @staticmethod
    def _описание_запроса(фильтры: str | None, сортировка: str | None) -> str | None:
        """Чем этот поиск отличался от соседних — словами, для человека."""
        из = {"описание": фильтры, "сортировка": сортировка}
        из = {к: з for к, з in из.items() if з}
        return json.dumps(из, ensure_ascii=False) if из else None
    ...
    async def записать_выдачу(self, разобранное: dict, *, ключ: str,
                              ...
                              фильтры: str | None = None, сортировка: str | None = None,
                              адрес: str | None = None) -> dict:
        ...
        async def записать():
            async with (await self.пул()).acquire() as c, c.transaction():
                запрос = await c.fetchval(
                    ...
                    self._описание_запроса(фильтры, сортировка),
